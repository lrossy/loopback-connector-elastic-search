...
ESConnector.prototype.buildFilter = function (model, idName, criteria, size, offset) {
    ...
    if (criteria) {
        ...
        if (criteria.routing) {
            filter.routing = criteria.routing;
            log('ESConnector.prototype.buildFilter.routing', 'discovered', JSON.stringify(filter.routing,null,0));
        }
    }
...
ESConnector.prototype.all = function all(model, filter, done) {
    ...
    var idName = self.idName(model);
    log('ESConnector.prototype.create', 'idName', idName);
    ...
    if(filter && filter.suggests) { // TODO: remove HACK!!!
        ...
        self.db.search(
            ...
            function (body) {
                ...
                var aggs = [];
                body.hits.hits.forEach(function (item) {
                    hits.push(self.dataSourceToModel(model, item, idName));
                });
                if(typeof body.aggregations !== 'undefined' ){
                    aggs = body.aggregations.agg1.buckets;
                }
                result.push(hits);
                result.push(aggs);
                log('ESConnector.prototype.all', 'model', model, 'result', JSON.stringify(result,null,2));
